<p align="right">
  <a href="./README.md">🇬🇧 English</a>
</p>

# 🚀 Halo, saya Muhammad Yusri.
Halo 👋  
Saya seorang backend-focused developer yang tertarik pada pengembangan sistem, arsitektur server, dan deployment berbasis container.

Saya aktif membangun aplikasi berbasis web, merancang dan mengelola database, serta melakukan deployment menggunakan Docker dalam lingkungan Linux.

---
<p align="center">
  <img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs?username=muhammadyusri121&show_icons=true&locale=en&layout=compact&theme=midnight-purple" height="170em"/>
  &nbsp;&nbsp;
  <img src="https://github-readme-stats-eight-theta.vercel.app/api?username=muhammadyusri121&show_icons=true&locale=en&theme=midnight-purple&include_all_commits=true&count_private=true" height="170em"/>
</p>

<p align="center">
  <a href="https://git.io/streak-stats">
    <img src="https://streak-stats.demolab.com?user=muhammadyusri121&theme=midnight-purple&fire=EBA108"/>
  </a>
</p>

---

## 💻 Teknologi yang Sering Digunakan

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,js,postgresql,mysql,react" />
</p>

**Fokus Utama:**
- Backend development  
- Perancangan REST API  
- Arsitektur database  
- Sistem backend yang scalable  
- Optimasi performa server  

---

## 📚 Sedang Dipelajari & Dieksplorasi

<p align="center">
  <img src="https://skillicons.dev/icons?i=elixir,flutter,azure,prisma" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/phoenix/phoenix-original.svg" height="48"/>
</p>

Saat ini sedang mendalami:

- Phoenix Framework untuk aplikasi web berperforma tinggi  
- Konsep concurrency pada Elixir  
- Infrastruktur cloud dan strategi deployment  
- ORM modern dan perancangan data  

---

## 🔧 Tools & Environment

<p align="center">
  <img src="https://skillicons.dev/icons?i=linux,docker,git,github,bash" />
</p>

Terbiasa bekerja dengan:

- Workflow berbasis terminal  
- Version control menggunakan Git  
- Development & deployment berbasis Docker  
- Konfigurasi VPS  
- Setup Nginx sebagai reverse proxy  


---

## 📫 Hubungi Saya

<p align="center">
  <a href="mailto:muhammadyusri00000@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/muhammadyusri121">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://linkedin.com/in/your-linkedin">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://instagram.com/y_usr1">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
  </a>
  <a href="https://facebook.com/alvin.alvan.126">
    <img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white"/>
  </a>
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/rtaserver/rtaserver/output/snake.svg" alt="Snake animation"/>
</p>
